...
export const LudoBoard: React.FC<LudoBoardProps> = ({ 
      ...
      for (let x = 0; x < 15; x++) {
        ...
        if (data.type === 'BASE') {
          if (!renderedBases.has(data.id)) {
            ...
            cells.push(
              <div key={key} className={clsx("col-span-6 row-span-6 border-[4px] md:border-[6px] border-ludo-dark relative z-0 rounded-3xl md:rounded-[2.5rem] shadow-sm md:shadow-inner transition-transform duration-500", data.color === 'red' ? 'bg-ludo-red' : data.color === 'green' ? 'bg-ludo-green' : data.color === 'blue' ? 'bg-ludo-blue' : 'bg-ludo-yellow')}>
                <div className="absolute inset-2 md:inset-4 bg-white/90 shadow-sm md:shadow-[inset_0_4px_12px_rgba(0,0,0,0.1)] grid grid-cols-2 grid-rows-2 gap-2 md:gap-4 p-3 md:p-5 rounded-2xl md:rounded-[2rem]">
                  {[0, 1, 2, 3].map((slotIdx) => {
                    ...
                    return (
                      <div key={slotIdx} className={clsx("flex items-center justify-center relative rounded-full border border-black/5", data.color === 'red' ? 'bg-ludo-red/10' : data.color === 'green' ? 'bg-ludo-green/10' : data.color === 'blue' ? 'bg-ludo-blue/10' : 'bg-ludo-yellow/10')}>
                        <div className="w-1/2 h-1/2 bg-white/40 rounded-full blur-none md:blur-[2px]" />
                        {pawn && isInBase && (
        ...
        if (data.type === 'CENTER') {
           if (x === 6 && y === 6) {
             cells.push(
               <div key={key} onClick={handleSecretClick} className="col-span-3 row-span-3 bg-ludo-dark relative border-[4px] md:border-[6px] border-ludo-dark active:scale-[0.98] transition-transform shadow-md md:shadow-2xl">
  ...
  return (
    <div className="aspect-square w-full h-full max-w-[650px] bg-ludo-dark p-2 md:p-4 shadow-xl md:shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)] relative select-none rounded-[2rem] md:rounded-[4rem]">
      <div className="grid grid-cols-15 grid-rows-15 w-full h-full bg-white border-2 md:border-4 border-ludo-dark rounded-[1.5rem] md:rounded-[3rem] overflow-hidden shadow-md md:shadow-2xl">
        {renderCells()}
      </div>
    </div>
  );
